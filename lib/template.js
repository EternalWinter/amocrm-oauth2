...
class Template {
    constructor(number, tokens, subdomain, call, responsibleId, statusId) {
        this.number = number;
    ...
    async get(key, query) {
        try {
            ...
            throw(`Error from template.get() : ${e}`);
        }
    }
    ...
    async post(key, query) {
        try {
            ...
            throw(`Error from template.post() : ${e}`);
        }
    }
